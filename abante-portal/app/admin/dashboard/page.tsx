...
import { supabase, Player, Payment, PlayerSummary, Division } from '@/lib/supabase'
...
export default function AdminDashboard() {
  ...
  const [playerForm, setPlayerForm] = useState({
    ...
    jersey_number: '', jersey_price: ''
  })
  const [paymentForm, setPaymentForm] = useState({
    player_id: '', amount_paid: '', payment_date: new Date().toISOString().split('T')[0],
  ...
  async function addPlayer(e: React.FormEvent) {
    ...
    setPlayerForm({ full_name: '', division: 'Mosquito', jersey_number: '', jersey_price: '' })
    fetchPlayers()
    setSaving(false)
  }
  ...
  async function deletePlayer(id: string) {
    if (!confirm('Delete this player and all their payments?')) return
  ...
  const inputClass =
    'w-full px-3 py-2 text-sm bg-white/5 border border-white/10 rounded-xl focus:outline-none focus:border-white/30 text-white placeholder:text-slate-600'
          ...
          {DIVISIONS.map((div) => (
            ...
            >
              ...
              <span className="ml-1.5 text-xs opacity-60">
                {players.filter((p) => p.division === div).length}
              </span>
            </button>
          ))}
        </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-sm">
            ...
            <form onSubmit={addPlayer} className="space-y-3">
              ...
              <div>
                <label className={labelClass}>Jersey Price (₱)</label>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-md max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setSelectedPlayer(null)} className="text-slate-500 hover:text-white">✕</button>
            </div>
